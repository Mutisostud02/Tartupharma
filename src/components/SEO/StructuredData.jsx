...
const StructuredData = ({ page = 'home', product = null, category = null }) => {
  ...
  const organizationSchema = {
    ...
    "description": "Leading supplier of pharmaceutical equipment in Kenya. Quality medical machinery for pharmaceutical manufacturing.",
    "address": {
      "@type": "PostalAddress",
  ...
  const getProductSchema = (productData) => ({
    ...
    "description": productData.description || `${productData.name} - Professional pharmaceutical equipment`,
    "manufacturer": {
      "@type": "Organization",
  ...
  const getCategorySchema = (categoryData) => ({
    ...
    "mainEntity": {
      ...
      "itemListElement": categoryData.products?.slice(0, 10).map((product, index) => ({
        "@type": "ListItem",
        "position": index + 1,
        "item": {
          "@type": "Product",
